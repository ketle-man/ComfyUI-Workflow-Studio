"""Video tab plan file management service (JSON plan + optional PNG index thumbnail).

Duplicated from LabPlanService rather than shared — the two plan formats are
unrelated (batch keyframe columns vs. sequential timeline blocks) and would
gain nothing from a forced common base beyond coincidental line-count savings.
"""
...
class VideoPlanService:
    ...
    def list_plans(self) -> list[dict]:
        ...
        for path in sorted(self.plan_dir.glob("*.json")):
            try:
                ...
                logger.warning("Skipping unreadable video plan %s: %s", path.name, e)
                continue
            ...
            if thumb_path.is_file():
                thumbnail = f"/wfm_data/video_plan/{stem}.png"
    ...
    def save_index_image_to_output(self, image_base64: str, filename_prefix: str = "Video_index") -> dict:
        """Saves the contact-sheet index image into ComfyUI's own output folder,
        alongside the videos generated during the run (unlike save_index_image,
