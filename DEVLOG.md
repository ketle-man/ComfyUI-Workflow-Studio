...
「Videoタブにバッチ動画生成機能を追加したい。既存の生成UIタブのLab機能を動画生成用に整えたイメージ」との要望を起点に着手。従来のVideoタブ（単発生成フォーム）をPlan/Asset/Editの3タブ構成に再設計し、複数セッションにわたるユーザー実機確認・レイアウト調整・追加要望への対応、および調査の過程で発見した重大バグの根本修正までを1リリースにまとめた。
...
**8. バグ修正: `__VideoAssets__`グループが再起動後に一覧・詳細パネルから消える**: 「再起動してギャラリーを確認したがグループに`__VideoAssets__`、`__vtemp__`が表示されなくなった」との報告。`gallery_metadata.json`を実データで直接確認したところ、`__vtemp__`はレジストリ・10件のメンバーシップとも正常だった一方、`__VideoAssets__`はグループレジストリ配列からも全画像のメンバーシップからも完全に消えていた（`delete_group()`はレジストリと全画像の`groups`配列を同時に消す実装で、症状と完全に一致）。原因は`__vtemp__`/`VideoPlan:<name>`と異なり、`__VideoAssets__`はバッチ実行側から`ensure_group`を呼ぶコードが一切無い設計だったこと — 3番で予約グループ保護（🔒表示・削除ボタン無効化）を追加する**前**の時点でManage Groupsから削除操作が可能だったため、その時点で削除され、以後`add_to_group`は個別画像への書き込みは成功してもレジストリを再登録しないため二度と一覧に出てこなくなっていた。`gallery-tab.js`に`ensureFeederGroup()`と同型の`ensureVideoGroup()`を新設し、VideoタブのAssetタブ初期化のたびに`__VideoAssets__`を確実に再登録するよう修正。削除時に失われた過去のメンバーシップ自体（開発中のテスト動画）はバックアップが無く復元不可能である旨をユーザーに報告した（このensure漏れが後に13番の伏線になる — `__VideoAssets__`側だけ直し`__vtemp__`側のensureが漏れていた）。

**9. Assetタブに保存済みPlan一覧を表示する「Project」サブタブを追加**: 「AssetタブにVideo Planを表示したい」との要望を起点に着手。何を指すか確認したところ「右ペインにプロジェクトタブを追加し、保存済み一覧、各動画のplan名表示としたい」との回答。調査の結果、バックエンドには`GET /api/wfm/video/plans`一覧APIと`/wfm_data/video_plan/{name}.png`サムネイル配信が2番のPlan永続化実装時から既に存在していたが、フロントエンドからは一度も呼ばれておらず丸ごと未使用のまま埋まっていたことが判明した。新規`video-project-tab.js`で保存済みPlan一覧（サムネイル・名前・ブロック数・更新日時）表示、クリックでのPlanエディタ読み込み（`video-plan-tab.js`に`openSavedVideoPlan()`をexport）、削除機能を実装し、Assetカードには生成元のPlan名をバッジ表示する機能（`img.groups`から`VideoPlan:<name>`グループを逆引き）も合わせて追加した。当初の指示通り右ペイン（Frame/GIFタブの隣）に実装したが、直後のフィードバックで「左ペインに変更したい」との指示を受け、左サイドバー（Assetタブの隣）へ丸ごと再配置した。この移動に伴い、左サイドバーのトグルロジックを「Assetボタン単体のshow/hide」から「クリックした方を開き他方を閉じる、同じボタンの再クリックで両方とも折りたたむ」という排他的トグルに拡張した。

**10. 中央下部のワークフロー名表示行を1行に統合**: 「Open Workflowボタン行を上段の現在読み込んでいるワークフロー名行と1行にしたい」との要望。`wfm-video-wf-name`（現在ロード中のワークフロー名）と`wfm-video-plan-workflow-row`（Planに記録されたワークフロー名+Open Workflowボタン）が縦に2行表示されていたのを、`.wfm-video-wf-row`という横並びflexコンテナでまとめ1行に統合した。

**11. ワークフロー設定可能項目ハイライト表示の復活**: 「以前の単発生成フォーム時の設定項目のハイライト表示を行いたい」との要望を受けて調査したところ、v0.5.4で一度実装されていた機能（`wfm-model-label-active`クラス、GenerateUI Modelタブと共通の設定色）が、4番のPlan/Asset/Edit再設計で単発フォームのコードごと失われ、ヘルプ文言だけが取り残されていたことが判明した。Planタブのブロックエディタ向けに`_updateFieldHighlights()`を再実装：Prompt/Duration/First Image/Last Image/Aspect Ratio/Megapixels/Multipleの各ラベルにIDを付与し、読み込んだワークフローが実際にそのフィールドをサポートしている場合のみハイライトする（Wan2.2のようにResolutionSelectorが無いワークフローではAspect Ratio/Megapixels/Multipleは非ハイライトのまま）。ワークフローロード時・Open Workflowボタン使用時・ブロック切り替え時（`_renderBlockEditor()`はブロック選択のたびにDOMを作り直すため都度再適用が必要）のいずれでも正しく再適用されるようにし、ハイライト色を注入するCSSセレクタ（`settings-tab.js`の`applyModelTabActiveColor()`）も旧レイアウトの`.wfm-video-form-panel`から新レイアウトの`.wfm-video-plan-content`に更新した（4番のレイアウト変更で参照先の無いセレクタになっていた）。この過程で、ヘルプ本文（`helpVideo8`）がHTML側とi18n.js側で別内容になっている既存の不整合（HTML側は旧ハイライト説明、i18n.js側はチェイン機能の説明で後者が実際に表示される）を発見し、ハイライト説明を新規`helpVideo10`として追加し直した。

**12. Video Assets/Video Tempグループの命名統一・自動登録の運用整理**: 「__vtempグループが消えてしまっている」という報告と、ユーザー自身による修正方針の提示（表示名をグループ名で統一、VideoPlan:自動登録を廃止し常にVideo Tempへ、VtCボタン名を「Clear __Video Temp__」に変更）を受けて実施。3番/8番で導入した`__VideoAssets__`/`__vtemp__`を`__Video Assets__`/`__Video Temp__`に改名し、Assetタブの表示名もグループ名からそのまま導出する形に統一（「Temporary (unsaved runs)」という独自表示名は廃止）。3番の「Plan保存済みなら`VideoPlan:<name>`サブグループへ」という自動振り分けは廃止し、Plan保存の有無に関わらず常に`__Video Temp__`へ登録するよう変更 — `__Video Assets__`への整理は完全にユーザー任意のタイミングに委ねる設計に転換した。対応してAssetタブの「VideoPlan:」動的取得・フィルタ表示ロジックと、9番で追加したAssetカードへのPlan名バッジ表示も、対応するグループ自体が無くなったため削除した。バックエンド予約グループリスト（`py/routes/gallery_routes.py`の`_RESERVED_GROUPS`）も新グループ名に追従させている。既存の`VideoPlan:`グループ自体はユーザー側で削除する前提で進めた。

**13. 重大バグ修正: `gallery_metadata.json`の非アトミック書き込みによるデータ消失（根本原因）**: 「再起動するとグループに登録した内容が消えます」との報告を受けた調査。まず8番の`ensureVideoGroup()`が実は`__VideoAssets__`（現`__Video Assets__`）しかensureしておらず`__vtemp__`（現`__Video Temp__`）はensureしていなかったという見落としを修正したが、実データ（`user/default/Workflow-Studio/gallery_metadata.json`、画像9,470件）を直接調べたところ、より深刻な問題が見つかった。`GalleryMetadataStore._save_to_disk()`が`open(path, "w")`による直接上書きの非アトミック書き込みで、バックグラウンドインデックス処理（`_bg_index_folder`、未キャッシュ画像10枚ごとに保存）が大量画像フォルダに対して極めて高頻度にこのファイルへ書き込み続けていた（実測でファイルサイズが0.5秒間隔で数百KB単位で変動し、外部プロセスから読むとタイミングによってJSONパースが失敗する状態だった）。この書き込みの最中にComfyUIプロセスが終了すると、ファイルが不完全なJSONのまま残り、次回起動時の`_load()`がパースに失敗して警告ログを出すだけで中身を空の`{"images": {}, "groups": []}`にリセットしてしまう — 以降1回でも保存が走ると、それまでの全グループ登録・タグ・メモが跡形もなく上書きされて消える、という構造だった。実際に確認したところ、グループの登録リスト自体（`__Feeder__`/`__Video Assets__`/`__Video Temp__`等）は残っていたが、動画がどのグループに属しているかという個々のメンバーシップ情報は失われていた。`_save_to_disk()`を一時ファイル書き込み+`Path.replace()`によるアトミックな置き換えに変更し、`_load()`がJSON破損を検出した場合は単に空データにするのではなく`.corrupt`ファイルとしてバックアップしてから起動するよう変更した（今後同様の事態が起きても手動復旧の余地を残す）。単体テスト（アトミック書き込み・破損時バックアップ・復旧後の再保存）で動作確認。今回失われた登録内容自体はバックアップが無く復元不可能である旨をユーザーに報告した。

**14. 中央プレビューの2ペイン化**: 手描きの構成図での要望「動画ソース・ファイルドロップエリアを左端に移動、プレビューを2つにして左はアセットの選択動画/ドロップされた動画のプレビュー兼用、右は生成された動画のプレビュー用」を実装。従来は単一のプレビュー（Video Sourceドロップの表示専用で、Assetタブの選択動画やPlan実行結果を自動表示する仕組み自体がそもそも存在しなかった）だったのを、3カラム構成（左: Video Sourceフォーム、中央: Asset選択/Sourceドロップ兼用プレビュー、右: 生成動画専用プレビュー、いずれも新規実装）に再構成した。`video-tab.js`・`video-asset-tab.js`（Asset選択）・`video-plan-tab.js`（バッチ実行結果）の3ファイルがプレビュー状態を共有する必要があったため、新規`video-preview.js`という共有モジュールに切り出した（video-tab.jsが既に他の2ファイルをimportしているため、逆方向のimportを追加すると循環参照になることを回避するため）。Frame/GIFツールは「直近にアクティブになった方のプレビュー」を対象に動作する設計にした。Asset選択時はGalleryの絶対パスをComfyUIのfilename/subfolder/type形式に変換する手間を避けるため、動画をBlobとしてfetchし「ローカルファイル」として扱うことで、GIF変換の既存アップロードロジックをそのまま流用できるようにした。Assetサイドバーの詳細パネルにあった動画プレビュー（`<video>`要素）は、中央プレビューに統合されたため削除した。

**15. ヘルプの整合性チェック**: リリース前の最終確認としてVideo Tabヘルプ全11項目（Title/Desc/1〜10）をHTML・i18n.js（3言語）・app.jsのhelpIdMapで突き合わせ、11番で発見した`helpVideo8`の不整合以外にも、`helpVideoDesc`（旧「Plan / Asset / Edit」の一括り記述が9番以降のサイドバー分離構造と食い違っていた）と`helpVideo1`/`helpVideo2`/`helpVideo3`（HTML側が旧単発フォーム時代の文言のまま放置され、実際に表示されるi18n.js側の内容と長年食い違っていた）を発見・修正した。README.mdのVideo Tabセクションも同様に、4番のPlan/Asset/Edit再設計自体が一切反映されないまま単発フォーム時代の説明で放置されていたことが判明し、9〜14番までの内容を反映して全面的に書き直した。

**16. `pyproject.toml`のlicense記述をComfy Registry仕様準拠の形式に修正**: リリース準備の一環で、`license = "MIT"`という単純文字列形式が、公式仕様（`license = {file = "LICENSE"}`または`{text = "..."}`のいずれか）に該当しない非推奨形式であり、`comfy env`実行時にcomfy-cliが警告を出していたことを確認した（[[project_registry_flagged_versions]]で追跡しているv0.5.0〜v0.5.3の原因不明のFlagged/Pending状態の調査依頼を受けて着手）。合わせて、参照先の`LICENSE`ファイル自体がリポジトリに存在しなかった（README.mdの「MIT License」という一文のみ）ため新規作成し、`license = { file = "LICENSE" }`に修正した。調査の結果、Comfy Registryのセキュリティスキャンは主にAI+静的解析（Ruff）によるPythonコード内の危険なパターン検出（`eval`/`exec`/`subprocess`/`pickle.loads`/カスタムpip wheel等）が対象で、`pyproject.toml`のメタデータ自体（license記述含む）がスキャン対象になるという情報は見つからなかった — つまり今回の修正はComfy Registryの公式仕様に沿った改善ではあるが、Flagged/Banned問題の直接的な解決策である確証はない。一方で、他の開発者（GitHub issue経由）でも同様の原因不明なFlaggedステータスが複数報告されており、pyproject.tomlの記述とは無関係にRegistry側の自動スキャンが誤検知（false positive）を起こしている可能性が高いことが分かった。具体例として、外部バイナリをsubprocessで呼び出す正当なコード（GraphicsMagick連携）が誤検知された報告があり、本プロジェクトにも同種のパターン（`gmic_routes.py`の`subprocess.Popen`+Windows固有の`CREATE_NEW_CONSOLE`フラグ、`wildcard_service.py`の`["cmd", "/c", "mklink", ...]`）が存在するため、これらが誤検知の一因になっている可能性はあるが確証はない（いずれも正当な機能で、変更は行っていない）。

**検証**: 全JS/Pythonファイルを`node --check`/`python -m py_compile`で構文確認、`templates/index.html`はPythonの`html.parser`でタグ対応チェックとJinja2実レンダリングで検証。ジャンクションバグの調査・検証は、ユーザーが起動中のComfyUI_5（ポート8189で稼働）に対し`curl`でAPIを直接叩いて実施 — `os.path.realpath()`で実際にジャンクションであることを確認し、`groups/{name}/images`（生成成功）と`list_images`（グループフィルタで空）の食い違いを再現、修正版デプロイ後の再検証はユーザー自身の実機確認で完了。8番の`__VideoAssets__`消失バグは、実行中サーバーの`gallery_metadata.json`を直接読み込んで原因を特定した（読み取り中にサーバー側の`_save_to_disk()`と競合しJSONDecodeErrorになることがあったため、パース成功するまで数回リトライして実データを確認）。13番の根本原因調査では、同じファイルへの読み取りを連続実行してもファイルサイズと成否が毎回変わることを確認し、実データが継続的に高頻度書き込みされている実態そのものを証拠として特定に用いた。13番の修正自体は`tempfile`を使った独立テストスクリプトで正常保存・再ロード後のデータ保持・破損ファイル検出時のバックアップ・復旧後の再保存を単体検証（ComfyUI起動不要）。開発元gitリポジトリと実行時`custom_nodes`フォルダは別実体のため（[[project_dev_deploy_sync]]参照）、変更・新規ファイルを都度両フォルダへ同期しCRLF/LF差異を除いた`diff`で完全一致を確認、Python変更（13番・12番の`gallery_routes.py`予約グループリスト）を含む回では実行時側の`__pycache__`もクリアした。各段階（レイアウト・Projectサブタブ追加・ハイライト復活・グループ運用整理・データ消失バグ・プレビュー2ペイン化・ヘルプ整合性）ともユーザー自身の実機確認・スクリーンショットや手描き構成図の提供を受けて次の修正に進む反復的な進め方で実施した。16番のRegistry仕様調査はWeb検索でComfy公式ドキュメント（`docs.comfy.org/registry/specifications`）とGitHub issue（Comfy-Org/ComfyUI-Manager #2927, #2997）を確認し、`pyproject.toml`の`license = { file = "LICENSE" }`変更後は`toml`パッケージでパース可能なことを確認した。

**How to apply**: 「◯◯機能では動くのに△△機能では動かない」という報告があった場合、両者の実行経路が本当に同じか（今回はLabがAPI形式を直接操作するのに対しVideoはUI形式から`convertUiToApi()`を経由する、という違い）を疑うと真因に早く到達できる。ComfyUIフロントエンドの`convertUiToApi()`のようなCOMBO値検証は「無警告でのフォールバック」を行う設計になっている場合があるため、実行時に動的アップロードした値をCOMBO項目に書き込む機能（画像チェイン等）を追加する際は、そのCOMBOの選択肢キャッシュが最新かどうかを必ず確認する必要がある。StabilityMatrixのような複数ComfyUIインスタンスでモデル/出力フォルダを共有するツールはジャンクション/シンボリックリンクを使うため、`Path.resolve()`をパスキーの一部に使うコードとファイルシステムスキャンで得たパスとの間に「解決する/しない」の不一致が生まれやすい — 同じ画像を指すパス文字列の保存/検索が全経路で一貫した規約（このプロジェクトでは「常にresolve()後の文字列で統一」）に従っているか、新しい書き込み経路を追加するたびに確認する価値がある。複数のタブ切替グループが同じCSSクラス/data属性を共有する実装は、後から片方だけ物理的に別の場所へ移動すると意図せず連動してしまうため、独立させたいグループは早い段階でクラス名やquerySelectorAllのスコープを分けておくとよい。予約グループ（`__Feeder__`のような「削除されると誰も自動で作り直さない」特別なグループ）を新設する際は、保護（削除禁止）の実装と同時に「常に存在することを保証する`ensure`呼び出し」もセットで用意しないと、保護を追加する前の空白期間や手違いで一度消えた場合に二度と復活しない — この2つは別々の対策であり、片方だけでは不十分。実行中サーバーが継続的に書き込んでいるJSONファイルを外部から直接読んで調査する際は、書き込みと衝突して不完全な内容を読んでしまうことがあるため、パースが安定して成功するまでリトライするのが安全。JSON等の設定/メタデータファイルへの永続化処理を新規実装・レビューする際は、単純な`open(path, "w")`による直接上書きになっていないか必ず確認する — 書き込み頻度が低いうちは問題が表面化しなくても、対象データ量が増えて書き込み回数が増えるほど「書き込み中のプロセス終了でファイルが壊れる」リスクが顕在化しやすく、かつ壊れたファイルのロード失敗を「空データにフォールバック」で処理していると、その後の1回の保存で過去の全データが消えるという最悪の組み合わせになる（一時ファイル+`Path.replace()`のアトミック置き換えと、破損検出時のバックアップ退避はセットで対策する）。UI再設計でコードごと削除した機能（今回のハイライト表示）は、対応するヘルプ文言だけが取り残されて「存在しない機能の説明」として残り続けることがあるため、大きめのUI再設計を行った際はヘルプ文言側の棚卸しも忘れずに行う。ユーザー向けドキュメント（README等）は実装が大きく変わった直後に更新を忘れると、次に触るまで長期間ズレたまま放置されやすい — 大規模なUI再設計コミットを作る際は、その場でREADMEの該当セクションも一緒に更新するのが安全（今回はPlan/Asset/Edit再設計そのものが数バージョン分READMEに未反映のまま残っていた）。
...
ユーザーからの複数の要望・不具合報告を1セッションで順に実装した一連の作業。

**1. LabタブPrompt列に「PG」ボタン — Promptタブのグループをキーフレームへ一括変換**: 「生成UIタブのLabのプロンプトにプロンプトタブのグループを使用できるようにしたい。'PG'ボタンを配置、モーダルでプロンプトのグループを選択、決定するとプロンプトのキーフレームが#1から自動で作成されます（#1は上書き）」との要望。`prompt-presets.js`の`pmGroups`（グループ名→プリセットID配列）を参照する`getPresetsInGroup()`を新設してexportし、`lab-tab.js`側にPresetManagerの一覧UI（`.wfm-pm-item`）を再利用したグループ選択モーダルを実装。選択→適用で`_lab.columns.prompt`をグループ内プリセット順に#1から全置換（#1は上書き、以降は#2,#3...）する。グループのプリセット数がBatch数を超える場合はBatch数を自動引き上げ（Batch入力欄自体の上限50でキャップ）、それでも収まらない分は既存の「Batchを超えたキーフレーム」警告トースト（`_warnIfKeyframesExceedBatch`）をそのまま再利用して通知する形にした。

**2. Lab: Model／Prompt／KSampler キーフレームの並べ替え**: 続けて「モデル、プロンプト、Ksamplerそれぞれでキーフレームの順変更を可能にしたい。キーフレームのモーダル展開をWクリックに変更しシングルクリックでキーフレームを選択、PGボタン左隣に上下ボタンを設置し操作可能にしたい」との要望。キーフレームセルのクリック挙動を分離し、シングルクリックは選択状態（列ごとに`_selectedIdx[col]`で管理、青枠でハイライト）、ダブルクリックで従来の編集モーダルを開くように変更。各列ヘッダーに▲／▼ボタンを追加し（Promptは要望通りPGボタンの左隣）、選択中のキーフレームと隣接するキーフレームの`atIteration`を入れ替えることで並び順を変更する実装にした。キーフレーム#1（ベースライン — GenerateUIのライブ値を表示する特別な行）は移動不可、かつ他のキーフレームが#1の位置に入れ替わることもないよう制約した。Prompt Group適用・Plan読込・Plan Clearなど列を丸ごと置き換える操作では選択状態を自動リセットするようにし、配列外を指す不整合を防いだ。

**3. バグ修正: デフォルトワークフローに設定した状態で同名上書き保存すると変更が反映されない**: 「生成UIタブで読み込んだデフォルトワークフローのモデルを変更しワークフローを同名で上書き保存した。ワークフロータブでJSONでモデルが変更していることを確認。ブラウザハード再読み込みを行うと以前のモデルが表示される」との不具合報告（スクリーンショット添付、旧モデル名がインストール済みモデルの一覧に無く「見つかりません」警告が出ている状態）。調査の結果、「デフォルトワークフローに設定」機能（`workflow-tab.js`）は設定した時点のワークフローJSON全体を`localStorage`（`wfm_settings.defaultWorkflowData`）へスナップショットとして保存し、起動時の自動読込（`generate-tab.js`）はこのスナップショットを読む仕様であることが判明。GenerateUIタブでの同名上書き保存はファイル自体は更新するが、このスナップショットまでは更新していなかったため、リロードのたびに保存前の古い内容が復活していた。`saveCurrentWorkflow()`の保存成功後処理に、保存したファイル名が現在の`defaultWorkflow`と一致する場合は`defaultWorkflowData`も同時に更新する処理を追加して修正した。

**4. Prompt Tab Tableビュー Styleタブに「+ Add to this file」対応**: 「プロンプトタブ、テーブルのスタイルにForm同様に既存のjsonファイルに追加する方法が欲しい（custom.json以外の場合）。該当jsonのスタイルを選択状態で+ Add to this fileボタンで追加できるようにしたい」との要望。Formタブのスタイルマネージャー（`prompt-styles.js`）には既にこの機能があったが、Tableビュー（`prompt-table.js`）の新規スタイル作成は常に`custom.json`固定（`styleApiCreate(..., null)`）だった。行選択が1件かつそのスタイルの`file`が`custom.json`以外の場合のみ有効化される「+ Add to this file」ボタンをツールバーに追加し、クリックすると保存先ファイルを保持したまま空の新規行を追加（File列に保存前からバッジ表示）、保存時に`styleApiCreate(name, pos, neg, pendingNewStyleFile)`で対象ファイルへ追記する実装にした。バックエンド（`settings_routes.py`）は元々`file`パラメータでの追記に対応済みだったため、フロントエンドの導線を追加するだけで実現できた。

**ヘルプ・README更新**: `project_v0336_conventions`の慣習（index.html + i18n.js 3言語 + app.jsのhelpIdMapの3点セット）に従い、Lab subtab（Setting/Columnsカードへ選択・並べ替えとPGボタンの説明2項目）とPrompt Tab（Table Viewカードへ「+ Add to this file」の説明1項目）を追加。README.mdはGenerateUI Tab（Saveボタン説明にデフォルトワークフロースナップショット修正の注記）、Lab subtab（見出しのバージョン範囲をv0.4.8へ更新、選択/並べ替えとPGボタンの新規項目）、Prompt Tab（Styleテーブルの項目に「+ Add to this file」のTableビュー対応を追記）を更新した。

**検証**: 全JSファイルを`node --check`で構文確認。開発元gitリポジトリと実行時`custom_nodes`フォルダは別実体のため（[[project_dev_deploy_sync]]参照）、変更ファイルごとに改行コード（CRLF/LF）の一致を確認してから同期し、都度`diff`で完全一致を検証した。デフォルトワークフローのバグ修正はユーザー自身の実機（GenerateUIでの上書き保存→ハード再読み込み）で確認済み。PGボタン・キーフレーム並べ替え・Styleの「+ Add to this file」もユーザー自身の実機確認で動作を確認済み。

**How to apply**: 開発元リポジトリと実行時`custom_nodes`フォルダへの同期前には必ず`file`コマンドで両ファイルの改行コードが一致しているか確認する（過去に一部ファイルだけLF化していた事例があるため — [[project_dev_deploy_sync]]参照）。一致していれば単純な`cp`で安全にデプロイでき、`diff`で完全一致を最終確認する。キーフレームや行の「並べ替え」機能を実装する際、内部的にソート順を保持する配列（今回は`atIteration`昇順）に対しては、配列そのものの並べ替えではなく「隣接要素同士でソートキーの値を交換する」実装にすると、他のロジック（`_resolveKeyframe`等）が前提とする「配列は常にソート済み」という不変条件を崩さずに済む。
...
v0.4.2リリース直後、ユーザーからLabタブに関する2件のフィードバックを受けて修正した一連のセッション。

**1. 共有モーダルのお気に入り☆・リネームがLabモーダルに残留するバグ**: ユーザーから「Labタブモーダルについて確認したい。各モーダルでお気に入り☆、名前変更が可能なようですがこれは機能していますか？」との指摘を受けて調査。Workflow Studioの全「詳細モーダル」（Workflow/Models/Nodesタブの詳細表示、Labのキーフレーム編集）は`#wfm-modal-overlay`というDOMを1つだけ共有しており、共通の`openModal()`（`app.js`）はタイトルの**テキストだけ**を書き換え、呼び出し元が動的に注入したヘッダー要素（お気に入り☆ボタン、タイトルクリックでリネームするイベントリスナー）は一切クリアしていなかった。そのため「Workflowタブでワークフローの詳細モーダルを開いた直後にLabのキーフレームモーダルを開く」と、直前のワークフロー用の☆ボタンとリネームリスナーがそのまま残留表示され、Lab側で☆をクリックすると**Labのキーフレームとは無関係に、直前に見ていたワークフローのお気に入り状態が実際に切り替わってしまう**ことをPlaywright MCPで実機再現・立証した。`openModal()`にクリーンアップ処理（`.wfm-fav-btn`の除去、タイトル要素を`cloneNode`で複製・差し替えて前の呼び出し元のクリックリスナー/属性を一掃、リネーム用隠し入力欄のリセット）を追加して修正。副次効果として、Workflow詳細モーダルを連続で複数回開いた際にタイトルのクリックリスナーが際限なく累積していく別の潜在バグ（`workflow-tab.js`側にはリネーム用clickリスナーの重複防止ガードが無かった）も同時に解消した。

**2. Lab Model列の＋／－が独立していなかった問題**: v0.4.2で実装したModel列（Checkpoint/LoRA/VAEを1つの`{checkpoint, lora, vae}`マージ行として1つのキーフレーム配列にまとめる設計）には設計上の欠陥があった。ユーザーから「checkpoint/lora/vaeいずれかで＋で追加すると他のモデルも追加されてしまいます。逆に-で他の２つも削除される」との指摘を受け、根本的な設計見直しを実施。マージ行方式ではなく、**Checkpoint・LoRA・VAEを（Prompt・KSamplerと全く同じ）完全に独立したキーフレーム配列に戻し**、C/L/Vトグルは「今どの配列を表示・操作するか」を切り替えるルーティングとしてのみ機能する設計に変更した。この結果、v0.4.2で新設した`_resolveModelKeyframe()`（3フィールドを1行にバンドルしたまま個別に前方向解決するカスタムロジック）が丸ごと不要になり、既存の汎用`_resolveKeyframe()`（Checkpoint/VAE列が元々使っていたもの）にLoRAを1つ追加するだけで済むようになった——継承機能・ハイライト・inheritedヒント表示など、v0.4.2で追加した機能はすべてこのシンプルな独立配列方式のままで自然に成立する。

**設計上の教訓**: 複数の独立した設定を1つのUI列にまとめる際、「見た目を1列にする」ことと「データを1行にマージする」ことは別の問題であり、後者を選ぶと今回のような「＋／－が意図せず道連れになる」バグを生みやすい。データモデルは独立させたまま、UI層（表示するフィールドの選択・共有される＋／－ボタンのルーティング）だけを1列に見せかける設計の方が、機能追加・バグ修正のコストが低く、直感的な挙動にもなりやすい。

**ヘルプ・README更新**: `project_v0336_conventions`の慣習に従い、Lab関連ヘルプ（`helpLab14`・`helpLab15`、英/日/中3言語＋`templates/index.html`の英語フォールバック）を独立配列方式の挙動に合わせて更新（特に`helpLab15`の「Revert to #1's settingは3項目まとめて戻る」という記述はもう正しくないため修正）。README.mdのLabセクションも同内容に更新した。

**検証**: 全JSファイルを`node --check`で構文確認。開発元gitリポジトリと実行時`custom_nodes`フォルダは別実体のため（[[project_dev_deploy_sync]]参照）、変更ファイルのみ両フォルダへ同期しハッシュ一致を確認。Playwright MCP経由で実機ブラウザ操作: (1) Workflow詳細モーダル→Labモーダルの順で開き、☆・リネームツールチップ・cursorスタイルが一切残留しないことを確認、(2) 「C」表示での＋／－がCheckpointのみに作用しLoRA/VAEは不変であること、「L」表示での＋／－がLoRAのみに作用しCheckpoint/VAEは不変であることをPlan JSONタブのデータ構造で確認、(3) v0.4.2形式（マージ行）のPlan JSONを読み込ませ、新しい独立配列形式へ正しくマイグレーションされることを確認。実際の画像生成を伴うRun実行は割愛。

**How to apply**:

1. DOM/モーダルを複数の呼び出し元で共有する設計では、各呼び出し元が「自分固有のUI要素」を動的に注入する場合、その呼び出し元自身が毎回クリーンアップするのではなく、**共有側の入り口関数（今回は`openModal()`）で毎回リセットする**設計にしないと、呼び出し順序に依存した残留バグ・誤操作バグを生む。cloneNode+replaceWithは、addEventListenerの参照を保持していなくてもリスナーを確実に一掃できる簡便な手段。
2. 複数の独立した設定を1つのUI要素（列・パネル等）にまとめる要望を実装する際、「データも1つにまとめる」のではなく「データは独立させたままUI層だけ共有する」設計を最初から検討する。今回はユーザーの2度のフィードバック（継承挙動の誤り→＋／－の誤り）を経てようやくこの設計にたどり着いたが、根本原因は同じ「フィールドを1行にバンドルした」という初期設計判断にあった。
...
ユーザーから「カタログ作成機能ですべてのスタイル（277）を選択して実行した149でバッチ処理が止まってしまった」との報告を受けて調査・修正した一連のセッション。合わせてStyle Catalogタブとカタログ作成機能、Promptタブへのスタイル管理機能追加も行った。

**1. バッチ処理が無言で停止するバグの修正（`comfyui-client.js`）**: 調査の結果、`trackProgress()`（WebSocket経由で生成完了を待つPromise）にタイムアウトが一切なく、`execution_interrupted`イベントも未処理だったことが根本原因と判明。ComfyUI側からの完了通知が何らかの理由で届かなくなると（WebSocket瞬断・生成中断など）、Promiseが永久に解決されないままバッチループの`await`が固まり、エラーも出さずに停止する状態になっていた。`_pendingTrackers`（promptId→reject関数のMap）を追加し、(a) `trackProgress()`に10分のタイムアウトを追加、(b) `execution_interrupted`イベントのハンドリングを追加、(c) WebSocket切断(`onclose`)時に待機中の全Promiseを即座にrejectする仕組みを実装。既存の`_runBatchLoop`（`generate-tab.js`）は元々「エラー時は次のアイテムへ進む」try/catch構造を備えていたが、`trackProgress()`が例外を投げないまま無限に待ち続けていたためこのcatchに到達できていなかった。今回の修正で、意図されていた挙動（1件失敗しても次へ進む）が正しく機能するようになった。停止した149件目（`MK shepard-fairey-style`）自体はJSON構文・エンコーディングとも正常で、スタイルデータ側に問題は無いことも確認済み。

**2. カタログ作成/バッチのスタイル処理順序の調査（コード変更なし）**: ユーザーからの追加質問（Batchタブの表示順とカタログ作成の生成順が異なる、149件目が停止する原因はファイルにあるか）に対して調査のみ実施。バックエンド`_load_styles()`（`settings_routes.py`）は`style/*.json`をファイル名のアルファベット順に読み込み、各ファイル内はJSON配列の記載順を維持する仕様であることを特定。Style Catalogギャラリーの表示順は生成順とは独立に常に`sort=name_asc`（ファイル名のアルファベット順）である仕様も確認。いずれも設計通りの挙動でありバグではないと結論。

**3. Style Catalogタブ: ワークフロー再利用ボタンの追加**: 「スタイルとして選択」ボタンの下に「Load in GenerateUI」「Open in Metadata Tab」の2ボタンを追加。当初「Outputフォルダの該当画像へ遷移するボタン」を検討したが、カタログ画像はOutput画像のバイト列そのままのコピー（`extra_pnginfo`のワークフローも含め完全に同一のPNGメタデータを保持）であるため、Output側の対応ファイルを探しに行かずカタログ画像自身から直接ワークフローを読み込む方式の方が、実装がシンプルな上に**既存の全カタログ画像に対しても即座に使える**（Output→カタログの対応を新たに記録する仕組みが不要）という利点があり、ユーザー確認の上でこちらを採用した。
...
- **ファイル名バッジ + 既存ファイルへの追加**（フォローアップ、ユーザーからの追加要望）: `_load_styles()`が各エントリに定義元ファイル名を`file`として付与するように変更し、一覧の各項目にファイル名を小バッジ表示。既存スタイル編集中は「+ Add to this file」ボタンが現れ、クリックするとフォームが空になり同じファイルへ追加される新規エントリとして保存できる（`POST /api/wfm/styles`の`file`パラメータで対象ファイルを指定、実在するファイルのみ許可しパストラバーサルを防止）。

**6. カタログ作成: Outputフォルダにコピーを残さないオプション**: カタログ作成は元々、ワークフローのSave Imageノードが生成の都度Outputフォルダへ通常通り書き出した上で、その画像をカタログフォルダへコピー保存する仕組みだった。大量スタイル（200件超）を一括作成するとOutputフォルダがほぼ重複ファイルで溢れる問題があり、モーダルに「Don't keep a copy in the Output folder」チェックボックスを追加。カタログへのコピー保存が**成功した後にのみ**、新設の`POST /wfm/gallery/output-image/delete`（filename/subfolder/typeからComfyUI Output配下の実ファイルを削除、`type="output"`以外は拒否）を呼び出してOutput側の元ファイルを削除する。コピー保存に失敗した場合は削除せず元画像を保持するため、データを失う経路が無い設計。既存の`GalleryService.delete_images()`（`_check_path_allowed`によるパス検証済み）をそのまま再利用しており、Outputフォルダ外のファイルを誤って削除する経路は存在しない。

**ヘルプタブ更新**: `project_v0336_conventions`の慣習（index.html + i18n.js 3言語 + app.jsのhelpIdMapの3点セット）に従い、GenerateUI/Prompt/Galleryタブヘルプに今回の追加項目をそれぞれ反映。
...
ユーザーからの実機フィードバックを受けて、Labタブに対して一連の修正・改善をまとめて実施した。

**1. VAEキーフレームが2回目・3回目以降に反映されないバグの修正**: ユーザーから「Labでチェックポイント焼き込みVAE構成のワークフローに対し、VAEキーフレームを2列目・3列目に追加して実行したが、生成画像のメタデータを確認するとVAEが変更されていなかった」との報告を受け調査。`_buildWorkflowForIteration()`（`lab-tab.js`）は`analysis.vae_nodes[0]`という「ワークフロー中の`VAELoader`ノードを機械的に拾っただけの最初の1件」に無条件で書き込む設計だったが、独立した`VAELoader`ノードを持たず、チェックポイントに焼き込まれたVAEをそのまま使う構成のワークフロー（実際にユーザーが使用した`simple_i2i_save.json`は`VAELoader`ノードが存在した通常構成だったため、このケース自体には該当しなかったが、独立VAELoaderが無いワークフロー全般に共通する潜在バグとして特定・修正）では`vae_nodes`が空になり、キーフレームで指定した値が警告なく黙って捨てられていた。`analysis.vae_nodes[0]`が存在しない場合、その回のワークフロー複製に`VAELoader`ノードを動的に追加し、`vae`入力を持つ全ノード（`VAEDecode`/`VAEEncode`等）の接続先をそのノードへ張り替えるフォールバック処理を追加した。`comfyUI.currentWorkflow`自体は変更せず、`_buildWorkflowForIteration()`が毎回作るクローンにのみ適用するため、他のGenerateUIサブタブには影響しない。

**2. Style適用UIの刷新**: これまでPromptキーフレームの「Style」欄は、上部GenerateUIツールバーの Style 有効化チェックボックスがONの場合のみ機能する「Apply」ボタン（クリックするたびに上部で選択中のスタイル名をボタンラベルに表示）という設計だった。ユーザー要望により、(a) 上部の Style 有効化チェックボックスの状態に関係なく動作するように変更、(b) モーダル内に専用のON/OFFチェックボックスを追加してキーフレームごとに適用状態を保持、(c) 適用したスタイル名はボタン上ではなく横の別要素に表示、(d) チェックボックスをOFFにした際はその名前表示のみクリアし、既にPositive/Negativeへ追記済みのテキストは自動削除しない（あくまでユーザーが手動管理）という仕様に変更した。実装の紆余曲折として、当初「チェックボックスをONにした瞬間に自動でスタイルを追記する」形で実装したが、ユーザーから「追加は（従来通り）Apply ボタンで行いたい」とフィードバックを受け、Apply ボタン押下時にのみ追記しチェックボックスを自動ON化する方式に修正した。適用状態（`styleApplied`/`styleName`）はキーフレームの`value`に永続化し、Plan保存/読込・モーダル再オープンでも復元される。

**3. Plan JSONタブのスクロール同期バグ修正**: Plan JSONタブのテキストエリアは、色付きシンタックスハイライト用の`<pre>`オーバーレイを背後に重ねる仕組みだが、他のRaw JSON表示（GenerateUIのRaw JSON等）と異なり`scroll`イベントリスナーの配線が漏れていたため、テキストエリアをスクロールしてもオーバーレイの表示位置が追従せず「スクロールしても表示が変わらない」ように見えるバグがあった。`json-highlight.js`の`syncScroll()`を`lab-tab.js`にimportし、`_initPlanJsonTab()`に`scroll`イベントリスナーを追加して解消した。

**4. キーフレームをモーダルから個別に削除可能に**: 各キーフレームの編集モーダルにある「削除」ボタンは、従来は列内の最後のキーフレームでのみ表示されていた。ユーザー要望により、1回目（baseline、常に固定）以外の全てのキーフレームで表示するよう変更（「−」ボタン自体は従来通り末尾のみ削除する仕様を維持）。途中のキーフレームを削除すると配列の`splice`により後続が自然に繰り上がる。

**5. Batch数超過時の警告トースト追加**: 「プランの適用回数よりBatch数が少なくなった場合はどうなるか」という質問を受けて挙動を調査したところ、Batch数を後から減らしてもキーフレームは削除・再採番されず、単に実行ループ（`for (let i = 1; i <= _lab.batchCount; i++)`）が届かなくなるだけで無警告だった（データは残るため、Batch数を再び増やせば復活する）。ユーザー要望を受け、`_maxKeyframeIteration()`で全列の最大`atIteration`を求め、現在のBatch数を超えていれば警告トーストを出す`_warnIfKeyframesExceedBatch()`を追加。Batch数入力欄の`change`イベントと、`_applyPlanData()`（Plan読込・Plan JSON Apply共通）の両方から呼び出す。

**6. Promptキーフレームでのワイルドカード対応**: 「プロンプトでワイルドカードを使いたい」との要望を受け、GenerateUIメインの生成フローが既に持っていたSPA側ワイルドカード展開処理（`generate-tab.js`の`_expandWildcardsInWorkflow()`、A1111形式`__name__`構文、`Workflow-Studio/wildcards/`配下の.txtから1行ランダム選択、ネストは最大5パス、`ImpactWildcardEncode`/`Processor`はサーバー側処理のため対象外）を`export`し、`lab-tab.js`の`_runLabBatch()`から各イテレーションのワークフロー構築直後・生成実行直前に呼び出すよう配線した。これによりPromptキーフレームのテキストに`__name__`を書いておけば、同じキーフレームを複数イテレーションで使い回していても毎回ランダムに再展開される。`generate-tab.js`は既に`lab-tab.js`から`initLabTab`をimportしており、今回の変更で双方向の循環importになるが、両関数とも関数宣言（hoisted）でトップレベル即時実行はしていないため問題なし（Node上での動的import検証でモジュールグラフの解決自体に問題がないことを確認済み）。

**7. 「Save index image to Output on Run」のデフォルトON化**: ユーザー要望により`_emptyLabState()`の初期値を`false`→`true`に変更。あわせて、修正過程で`_clearPlan()`が`saveIndexEl.checked`を無条件に`false`へ固定していた（内部状態とチェックボックス表示が食い違う潜在バグ）ことに気づき、`_lab.saveIndexOnRun`の実値を反映するよう修正した。既存の保存済みPlanファイルは`save_index_on_run`の値をそのまま保持するため影響を受けない。

**検証**: 各修正・機能追加ごとに`node --check`で構文検証、開発元リポジトリと実行時`custom_nodes`フォルダの`diff`比較で意図した差分のみであることを確認してから同期。VAE修正は`analyzeWorkflow()`の実データ（ユーザー提供の`simple_i2i_save.json`）に対するNode単体実行で`vae_nodes`検出ロジックを確認済み。循環import追加分はNode上で`import()`によるモジュール解決を検証済み（`window is not defined`はNode実行環境固有の話で、モジュールグラフ解決自体は正常に完了することを確認）。

**How to apply**: (1) `analysis.*_nodes[0]`のような「先頭要素決め打ち」パターンは、そのノード種別がワークフロー中に必ず1つ存在する保証がない限り、存在しないケースへのフォールバックを用意すること（VAELoaderのように「省略可能」なノードは特に注意）。(2) UIのON/OFFチェックボックスとその内部状態(`_lab.xxx`)を分離して持つ場合、リセット系の関数（`_clearPlan()`等）で値をハードコードせず、必ず内部状態から読み出すこと——デフォルト値を変更した際に無言でズレるバグの温床になる。(3) `generate-tab.js`と`lab-tab.js`は既に循環importの関係にあるため、新たに一方から他方の関数を使う場合はトップレベルで即時評価される値（`const`の即時代入等）を挟まないよう注意する（関数宣言はhoistされるため安全）。
...
## 2026-06-29: v0.3.66 — Mask ツール 5サブツール追加（Color / Alpha / Text / Vector / Shape）・Support リンク追加

**変更ファイル**: `static/js/image-edit/MaskEditorOneTools.js`（新規）, `static/js/image-edit-tab.js`, `templates/index.html`, `static/js/i18n.js`, `static/js/app.js`

### 概要

Image Edit タブの Mask ツールに Color / Alpha / Text / Vector / Shape の5つのサブツールを追加。いずれも Mask Editor One に依存せず単独で動作する。また Help の Support ページに comfyui-image-feeder と comfyui-mask-editor-one のリポジトリリンクを追加。

---

### MaskEditorOneTools.js（新規）
...
- `feather`（0–100%）: tolerance 範囲内で距離に応じた不透明度グラデーション
...
- 複数行対応（textarea 内改行 → 行ごとに `fillText`）
- Ctrl+Enter / Stamp ボタンで確定、Escape / Cancel でキャンセル
...
- `_buildSpline(ctx, pts, closed)`: Catmull-Rom → Bézier 変換して `bezierCurveTo` で描画
- `onBeforeCommit` コールバック: コミット直前に Undo スナップショットを取得
...
- **ストロークバッファ問題対応**: `_curX / _curY / _curShift` を `onMouseMove` 時に記録し、`onMouseUp(x, y, e)` で `x ?? this._curX` を使用（`_onToolMouseUp` が `refCanvas` 座標を持たないため）
- `mode`: `"add"` / `"erase"`、`shape`: `"rect"` / `"ellipse"`
...
- Text: Font セレクト・Size 入力・Bold/Italic/Align・Mode セレクト
- Vector: 点数表示・Mode セレクト・Undo ヒント
...
- `_onToolMouseLeave`: `vector` → `vectorTool.onMouseLeave()`
...
## 2026-06-29: v0.3.65 — Mask Editor One 連携修正・トップバーアイコン修正・Mask ABRブラシ対応

**変更ファイル**: `static/js/image-edit-tab.js`, `static/js/image-edit/MaskTool.js`, `web/comfyui/top_menu_extension.js`, `custom_nodes/comfyui-mask-editor-one/server.py`（インストール済み）

**Mask Editor One 対応バージョン**: v0.1.9+（BiRefNet / SAM3 / ABRブラシライブラリ連携）

### 概要

Mask Editor One の仕様変更に伴う連携修正、プロパティパネル開閉によるトップバーアイコン消滅バグの修正、および Mask ツールへの ABR ブラシサポート追加。
...
> **Mask Editor One リポジトリ側**: 開発元 `server.py` にも同修正を適用済み。PR 時に含めること。
...
- `MutationObserver` を追加して Vue 再レンダリング後も即座に SVG を再適用
  - `btn.querySelector("i")` で `<i>` リセットを検知 → `applyIconToButton` 再実行
  - 監視ノード: `[data-testid="action-bar-buttons"]` → `.actionbar-container` → `document.body`（フォールバック順）
  - Observer 参照を `window.__wfmIconObserver` に保持

```javascript
// 旧: 一度だけ適用、Vue 再レンダリングで消滅
const replaceButtonIcons = () => { ... btn.innerHTML = cfg.getIcon(); };

// 新: MutationObserver で監視・再適用
const applyIconToButton = (btn, cfg) => {
    if (btn.querySelector("svg")) return;  // 既存なら skip
...
### Mask ツール — ABRブラシ対応（Mask Editor One オプション）

Mask Editor One がインストールされている場合のみ有効。未インストール時は従来の円形ブラシのみで動作（機能維持）。
...
- `_getImageStamp()`: アルファチャンネル自動判定（ABR 透過 PNG 対応、グレースケール PNG は輝度から逆算）
- `_paintImageBrush()`: `angle` 非ゼロ時に translate + rotate で描画
- `_paintToStroke()`: Jitter 有効時は各スタンプに適用後、状態をリストア（PaintTool.js 同様）
...
**プロパティパネル（Paint サブツール）**

```
Mode     [Add] [Erase]
Size     ━━━━━━  30px
Hardness ━━━━━━  85%   ← ABRブラシ選択中は disabled
─── MASK EDITOR ONE ───
Brush    Circle   [Select] [✕]
 ↓ ブラシ選択後
Spacing  ━━━━━━  25%
...
- **Select ボタン**: ABR ブラシライブラリピッカーモーダルを開く
- **✕ ボタン**: Circle（デフォルト）に戻す（ブラシ選択中のみ表示）
...
- ブラシクリック → `setImageBrush()` 適用後モーダル閉じる
...
- **Checkpoint/LoRA/Prompt/Workflow 全バッチグループ**: v0.3.55 で誤って追加した予約グループ（Batch/Stack）除外フィルターを削除 → Batch/Stack グループはバッチ実行UIに表示され正常に選択可能
...
Applyボタン下部に固定サイズボタンを追加。クリックするとWidth・Height両方に同じ値をセットする。

- `512` `768` `1024` `2048` — よく使う正方形サイズ
- `1K`(1024) `2K`(2048) `4K`(4096) — 映像規格対応の正方形サイズ

**解像度ドロップダウン + WHSet / HWSet ボタン**

ドロップダウンからアスペクト比プリセットを選択し、横並びの2ボタンでWidth/Heightにセット。

- `WHSet` — 選択値をそのままWidth/Heightにセット（例: 720x1280 → W=720, H=1280）
- `HWSet` — W/Hを入れ替えてセット（例: 720x1280 → W=1280, H=720）

デフォルト10件（削除不可）: `720x1280` `768x1024` `1152x896` `1344x768` `832x1216` `832x1248` `832x1280` `1920x1080` `2560x1440` `3840x2160`

**カスタムプリセット管理（+/-ボタン）**

- `+` — 現在のWidth/Height値をカスタムプリセットの先頭に追加。デフォルトや既存カスタムと重複する場合は無視
- `−` — 選択中のカスタムプリセットを削除。デフォルト行は削除不可（`data-default="1"` で判定）

カスタムプリセットは `wfm_latent_presets` キーでlocalStorageに永続化。ドロップダウンはカスタム→デフォルトの順で表示。
